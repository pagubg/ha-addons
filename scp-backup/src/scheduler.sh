...
# Execute transfer
transfer_output=$(transfer_all_backups "$SSH_HOST" "$SSH_PORT" "$SSH_USER" "$REMOTE_PATH" "$TRANSFER_TIMEOUT" "$VERIFY_TRANSFER" "$KEEP_LOCAL_BACKUP" "$DELETE_AFTER_DAYS" "$BACKUP_NAME_PREFIX" 2>&1)
transfer_result=$?
